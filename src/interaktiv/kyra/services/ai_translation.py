...
from interaktiv.kyra.services.translation_marker import mark_ai_translated
...
def _apply_translation(obj, payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for item in targets:
        ...
        # Mark the target as machine-translated / not yet editorially reviewed.
        # Must be the last write to `existing` so the stored modified timestamp
        # reflects the fully translated state (see translation_marker module).
        try:
            mark_ai_translated(existing, source_lang, gateway_available)
        except Exception as exc:
            logger.warning("[KYRA AI TRANSLATE] could not set AI marker: %s", exc)
